...
export default async function (client: Client, { user, guild, args, actions }: CommandParamaters) {
    ...
    const response = await fetch(env.PELICAN_API + "/client/servers/" + env.PELICAN_UUID + "/command", {
        ...
        body: JSON.stringify({ command: `whitelist remove ${username.value}` }),
    });

    if (response.status !== 204) return await actions.reply({ content: "couldn't remove user from whitelist" });

    return await actions.reply({ content: "removed user from whitelist" });
}
